<?php
	...
	*/
	...
	// create admin menu
	add_action('admin_menu', 'gdlr_lms_add_admin_menu', 99);
	...
	//add_action('gdlr_save_hotel_option', 'gdlr_generate_hotel_style_custom');
	//if( !function_exists('gdlr_generate_hotel_style_custom') ){
	//	function gdlr_generate_hotel_style_custom( $options ){
	//		
	//	}
	//}
						...
						'options' => array(
							...
							'booking-option' => array(
								...
								'options' => array(
									...
									'special-season-date' => array(
										'title' => __('Special Season Date', 'gdlr-hotel'),
										'type' => 'textarea',	
										'default' => '',
										'description' => __('Fill the date in Y-m-d format. Use * for recurring date separated each date using comma.', 'gdlr-hotel') . '<br>' .
											__('Ex. *-12-25 means special season is running every Christmas. 2015-7-* covers every day on July.', 'gdlr-hotel')
									),
