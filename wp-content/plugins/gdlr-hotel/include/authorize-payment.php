...
if( !empty($_GET['invoice']) && !empty($_GET['response']) && $_GET['response'] == 1 ){
	...
	if($response->isAuthorizeNet()){
		...
		if($response->approved){
			...
			$redirect_url = esc_url(add_query_arg(array($hotel_option['booking-slug']=>'', 'state'=>4, 'invoice'=>$_GET['invoice']), home_url()));
		}else{
			$redirect_url = esc_url(add_query_arg(array($hotel_option['booking-slug']=>'', 'state'=>4, 'invoice'=>$_GET['invoice'],
